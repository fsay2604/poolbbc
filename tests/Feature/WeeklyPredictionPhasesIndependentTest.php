...
class WeeklyPredictionPhasesIndependentTest extends TestCase
{
    ...
    public function test_user_can_confirm_a_prediction_where_hoh_is_also_a_nominee_veto_winner_and_evicted(): void
    {
        ...
        $this->assertSame($boss->id, $prediction->hoh_houseguest_id);
        $this->assertSame($boss->id, $prediction->nominee_1_houseguest_id);
        $this->assertSame($boss->id, $prediction->veto_winner_houseguest_id);
        $this->assertSame($boss->id, $prediction->evicted_houseguest_id);
    ...
    public function test_admin_can_save_an_outcome_where_hoh_is_also_veto_winner_and_evicted(): void
    {
        ...
        $response = Livewire::test('admin.weeks.outcome', ['week' => $week])
            ->set('form.boss_houseguest_ids.0', $boss->id)
            ->set('form.nominee_houseguest_ids.0', $boss->id)
            ->set('form.nominee_houseguest_ids.1', $other->id)
            ->set('form.veto_winner_houseguest_id', $boss->id)
            ->set('form.veto_used', true)
            ->set('form.saved_houseguest_id', $other->id)
            ->set('form.replacement_nominee_houseguest_id', $other->id)
            ->set('form.evicted_houseguest_ids.0', $boss->id)
            ->call('save');
    ...
    public function test_admin_can_save_a_prediction_where_hoh_is_also_evicted(): void
    {
        ...
        $response = Livewire::test('admin.predictions.edit', ['prediction' => $prediction])
            ...
            ->set('form.veto_used', true)
            ->set('form.saved_houseguest_id', $other->id)
            ->set('form.replacement_nominee_houseguest_id', $hg3->id)
            ->set('form.evicted_houseguest_ids.0', $boss->id)
            ->call('save');
